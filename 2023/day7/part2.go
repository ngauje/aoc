...
func getHandType(cards []byte) int {
	...
	four := 0
	five := 0
	jokers := strings.Count(string(cards), "J")
	...
	} else if four == 1 && jokers == 4 {
		return 6 // Five of a kind: 8JJJJ
	} else if four == 1 && jokers == 1 {
		return 6 // Five of a kind: AAJAA
	} else if four == 1 {
		return 5 // Four of a kind: AA8AA
	} else if (brelan == 1 && pairs == 1) && (jokers == 2 || jokers == 3) {
		return 6 // Five of a kind: 23332 or J333J or 2JJJ2
	} else if brelan == 1 && pairs == 1 {
		return 4 // Full house: 23332
	} else if brelan == 1 && jokers == 3 {
		return 5 // Four of a kind: JJJT8
	} else if brelan == 1 && pairs == 0 && jokers == 1 {
		return 5 // Four of a kind: TTTJ8
	} else if brelan == 1 && pairs == 0 {
		return 3 // Three of a kind: TTT98
	} else if pairs == 2 && jokers == 2 {
		return 5 // Four of a kind: 23JJ2
	} else if pairs == 2 && jokers == 1 {
		return 4 // Full house: 23J32
	} else if pairs == 2 {
		return 2 // Two pair: 23432
	} else if pairs == 1 && jokers == 2 {
		return 3 // Three of a kind JJ842
	} else if pairs == 1 && jokers == 1 {
		return 3 // Three of a kind AJ3A4
	} else if pairs == 1 {
		return 1 // One pair: A23A4
	} else if pairs == 0 && jokers == 1 {
		return 1 // One pair: J358Q
	} else {
		return 0
	}
}
...
func main() {
	...
	for i := 0; i < len(hands); i++ {
		...
		// fmt.Printf("Hand: %s (%d)\n", hands[i].cards, hands[i].handType)
	}
